...
// VOICE ID YANG BENAR UNTUK BELLA
const VOICE_IDS = {
    bella: "hpp4J3VqNfWAUOO0d1Us", // Bella - Perempuan (ID BENAR)
...
// ==================== AUDIO GENERATION DENGAN SUARA BELLA (ID BENAR) ====================
async function generateAudio(text, useMaleVoice = false, style = 0.0, speed = 1.0) {
    if (!elevenEnabled) {
        const audioBox = document.getElementById('audioBox');
        audioBox.innerHTML = `
    ...
    // MENGGUNAKAN VOICE BELLA (ID BENAR) UNTUK PEREMPUAN, ADAM UNTUK COWOK
    const voiceId = useMaleVoice ? VOICE_IDS.adam : VOICE_IDS.bella;
    const voiceName = useMaleVoice ? "ADAM (Male)" : "BELLA (Female)";
    
    // Tentukan nama style berdasarkan nilai
    let styleName = "Normal";
    if (style > 0.7) styleName = "🤯 Extreme";
    else if (style > 0.5) styleName = "🔥 Excited";
    else if (style > 0.3) styleName = "😲 Shocked";
    else if (style > 0.1) styleName = "😊 Happy";
    else if (style < -0.3) styleName = "🫢 Teasing";
    else if (style < -0.6) styleName = "😴 Tired";
    else if (style < -0.8) styleName = "😭 Sad";
    
    // Tentukan deskripsi speed
    let speedDesc = "Normal";
    if (speed > 1.2) speedDesc = "Very Fast";
    else if (speed > 1.1) speedDesc = "Fast";
    else if (speed < 0.9) speedDesc = "Slow";
    else if (speed < 0.8) speedDesc = "Very Slow";
    ...
    try {
        // Style bisa diatur dari -1 sampai 1
        const voiceSettings = {
            stability: Math.max(0.2, Math.min(0.8, 0.5 - (style * 0.15))),
            similarity_boost: 0.75,
            style: Math.max(-1, Math.min(1, style)),
            speed: Math.max(0.7, Math.min(1.3, speed)),
            use_speaker_boost: true
        };
        
        // MENGGUNAKAN ELEVEN TURBO V2.5 (HEMAT KREDIT)
        ...
        if (audioPlayer) {
            ...
            const audioBox = document.getElementById('audioBox');
            audioBox.innerHTML = `
                <div style="font-size: 11px; font-weight: 800; margin-bottom: 8px; color: #6b7280;">🔊 ${voiceName} [${styleName}] [${speedDesc}] (Turbo)</div>
