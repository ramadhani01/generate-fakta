...
// Fallback jika parsing gagal - PASTIKAN STYLE LOCK DI AWAL
if (!textToImage) {
    textToImage = `${STYLE_LOCK_SKELETON} Adegan ${sceneText}. Karakter kerangka humanoid dengan lapisan transparan, ${framing}, full body visible. Pencahayaan dramatis.`;
}

if (!imageToVideo) {
    imageToVideo = `${STYLE_LOCK_SKELETON} Adegan ${sceneText}. Karakter kerangka humanoid dengan lapisan transparan, ${framing}, full body visible. Pencahayaan dramatis. Gerakan lambat, transisi fade out.`;
}

// Pastikan STYLE LOCK ada di awal (perbaikan jika AI tidak meletakkan di awal)
if (!textToImage.startsWith(STYLE_LOCK_SKELETON.substring(0, 30))) {
    textToImage = `${STYLE_LOCK_SKELETON} ${textToImage}`;
}

if (!imageToVideo.startsWith(STYLE_LOCK_SKELETON.substring(0, 30))) {
    imageToVideo = `${STYLE_LOCK_SKELETON} ${imageToVideo}`;
}
