...
// LOGIKA BARU: Mengontrol kapan otot muncul agar tidak menutupi tulang di awal
let anatomyDetail = "";
if (sceneType === "HOOK") {
    anatomyDetail = "Strict focus on the WHITE SKELETAL structure. NO MUSCLES. The space between bone and glass skin is empty and transparent.";
} else if (sceneType === "PUNCAK EFEK") {
    anatomyDetail = "Intense red anatomical muscle fibers appearing under the glass skin to show physical strain, but the white skeleton is still clearly visible through the muscles.";
} else {
    anatomyDetail = "Clear glass body showing the internal skeleton with very thin, faint muscle fibers starting to form.";
}
...
FRAMING: ${framing}
ANATOMY RULE: ${anatomyDetail}

WAJIB IKUTI URUTAN INI dalam MENYUSUN PROMPT:
...
1. Mulai dengan deskripsi visual yang SAMA dengan TEXT TO IMAGE

2. Tambahkan GERAKAN spesifik yang terjadi (tangan gemetar, kepala menunduk, tubuh bergoyah, dll)

3. Tambahkan DURASI gerakan (slow motion, normal speed, fast motion)

4. Tambahkan TRANSISI ke scene berikutnya (fade out, cut, dissolve)

5. Tambahkan EFEK KAMERA (zoom in/out, pan, tilt, handheld shake)

6. TERAKHIR, tambahkan STYLE LOCK yang sama
...
TEXT TO IMAGE:
[prompt text to image lengkap dalam satu paragraf]

IMAGE TO VIDEO:
[prompt image to video lengkap dalam satu paragraf]`;
...
if (!textToImage) {
    textToImage = `Adegan ${sceneText}. Karakter kerangka humanoid dengan lapisan transparan, ${framing}, full body visible. Pencahayaan dramatis. ${STYLE_LOCK_SKELETON}`;
}

if (!imageToVideo) {
    imageToVideo = textToImage + ` Dinamis dengan gerakan lambat, transisi fade out.`;
}
