...
Tugas: Buat prompt TEXT TO IMAGE dan IMAGE TO VIDEO yang terpisah dan SANGAT DETAIL untuk adegan berikut.
...
1. ADEGAN:
   - Deskripsi situasi secara keseluruhan
   - Apa yang terjadi dalam adegan ini
   - Latar belakang/lingkungan yang relevan dengan narasi

2. POSE:
   - Posisi tubuh kerangka secara detail
   - Bahasa tubuh yang mencerminkan kondisi (lelah, tegang, kacau)
   - WAJIB full body visible (kepala sampai kaki)

3. KAMERA:
   - ${sceneType === 'HOOK' ? 'Boleh medium shot atau close-up untuk hook' : 'WAJIB wide shot (seluruh tubuh terlihat)'}
   - Sudut kamera (eye level, low angle, high angle)
   - Jarak kamera
...
4. LIGHTING:
   - Sumber cahaya
   - Suasana pencahayaan (dramatis, remang, terang)
   - Kontras dan bayangan

5. MOOD:
   - Suasana hati yang ingin dicapai
   - Perasaan yang muncul saat melihat adegan

6. STYLE LOCK:
${STYLE_LOCK_SKELETON}

WAJIB:
- Lingkungan HARUS relevan dengan narasi (bukan ruangan kosong generik)
- Jika narasi menyebutkan tempat tertentu, gunakan tempat itu
- Full body visible untuk semua scene kecuali hook boleh medium shot
- Semua elemen visual harus mendukung narasi

Output HARUS dengan format EXACT:

TEXT TO IMAGE:
[prompt text to image lengkap dengan format 1-6 di atas]

IMAGE TO VIDEO:
[prompt image to video lengkap - TAMBAHKAN instruksi gerakan spesifik, durasi, dan transisi]`;

    // Gunakan userPrompt yang lebih kuat
    const raw = await callGroq(
        `Buat prompt visual lengkap dan detail untuk adegan berikut:\n${sceneText}\n\nTipe adegan: ${sceneType}`,
        systemVisual
    );
    ...
    // Fallback jika parsing gagal - buat manual dengan format yang benar
    if (!textToImage) {
        textToImage = `1. ADEGAN: ${sceneText}\n2. POSE: Kerangka berdiri dengan postur ${sceneType === 'PUNCAK EFEK' ? 'goyah tidak stabil' : 'tegang'}, full body visible.\n3. KAMERA: ${sceneType === 'HOOK' ? 'Medium shot' : 'Wide shot'}, sudut eye level.\n4. LIGHTING: Dramatis dengan bayangan tajam.\n5. MOOD: Mencekam dan menegangkan.\n6. STYLE LOCK: ${STYLE_LOCK_SKELETON}`;
    }
    
    if (!imageToVideo) {
        imageToVideo = `${STYLE_LOCK_SKELETON}\n\nVideo sinematik dari: ${sceneText}\nGerakan: ${sceneType === 'PUNCAK EFEK' ? 'Tubuh goyah dan gemetar perlahan' : 'Gerakan statis dengan sedikit perubahan posisi'}\nDurasi: 8 detik\nTransisi: Fade to black.`;
    }
            ...
            sceneHtml += `
                <div class="scene-item skeleton-mode" data-scene="${i}">
                    ...
                    <div class="prompt-section">
                        ...
                        <div class="prompt-content" id="tti-${i}">${prompts.textToImage.substring(0, 150)}${prompts.textToImage.length > 150 ? '...' : ''}</div>
                    ...
                    <div class="prompt-section" style="margin-top: 12px;">
                        ...
                        <div class="prompt-content" id="itv-${i}">${prompts.imageToVideo.substring(0, 150)}${prompts.imageToVideo.length > 150 ? '...' : ''}</div>
                        <div class="scene-actions">
