...
const systemVisual = `Anda adalah sutradara video AI yang membuat prompt sinematik.

Tugas: Buat SATU PARAGRAF PROMPT VISUAL LENGKAP untuk adegan berikut.
...
WAJIB IKUTI URUTAN INI dalam MENYUSUN PROMPT (TANPA LABEL):

1. Deskripsi ADEGAN secara visual (APA yang terjadi, di mana lokasinya, apa yang dilakukan karakter)

2. POSE dan bahasa tubuh karakter (kerangka humanoid dengan lapisan transparan)

3. FRAMING kamera (${framing}, full body visible, jangan close-up wajah)

4. PENCAHAYAAN (suasana, sumber cahaya, intensitas)

5. MOOD dan atmosfer yang ingin dicapai

6. TERAKHIR, tambahkan STYLE LOCK ini secara utuh: ${STYLE_LOCK_SKELETON}
...
Tulis SATU PARAGRAF sinematik yang menggabungkan semua elemen di atas.

Jangan gunakan daftar.

Jangan gunakan penomoran.

Jangan gunakan label seperti "ADEGAN:", "POSE:", dll.

Output hanya teks prompt final siap pakai.`;
...
// Bersihkan prompt dari kemungkinan label yang masih tersisa
let cleanPrompt = raw
    .replace(/^ADEGAN:?\s*/gmi, '')
    .replace(/^POSE:?\s*/gmi, '')
    .replace(/^FRAMING:?\s*/gmi, '')
    .replace(/^KAMERA:?\s*/gmi, '')
    .replace(/^PENCAHAYAAN:?\s*/gmi, '')
    .replace(/^LIGHTING:?\s*/gmi, '')
    .replace(/^MOOD:?\s*/gmi, '')
    .replace(/^SUASANA:?\s*/gmi, '')
    .replace(/^ATMOSFER:?\s*/gmi, '')
    .replace(/^STYLE LOCK:?\s*/gmi, '')
    .trim();

// Pastikan STYLE LOCK ada di akhir
if (!cleanPrompt.includes(STYLE_LOCK_SKELETON.substring(0, 50))) {
    cleanPrompt = cleanPrompt + ' ' + STYLE_LOCK_SKELETON;
}

// Untuk kompatibilitas dengan sistem yang ada, kita tetap buat TTI dan ITV
// Tapi sebenarnya prompt visual sudah lengkap sendiri
return { 
    textToImage: cleanPrompt,
    imageToVideo: cleanPrompt + ' Dinamis, gerakan lambat, transisi halus.',
...
try {  
        ...
        sceneHtml += `  
            <div class="scene-item skeleton-mode" data-scene="${i}">  
                ...
                <div class="prompt-section">  
                    <div class="prompt-label">🖼️ VISUAL PROMPT</div>  
                    <div class="prompt-content" id="prompt-${i}">${prompts.fullPrompt.substring(0, 150)}${prompts.fullPrompt.length > 150 ? '...' : ''}</div>  
                    <div class="scene-actions">  
                        <button onclick="if(typeof copyTextToImage==='function')copyTextToImage(${i})" class="copy-tti">📋 Copy Prompt</button>  
    ...
    // Tampilkan tombol COPY ALL PROMPTS  
    const copyPromptsBtn = document.getElementById('copyPromptsBtn');  
    if (copyPromptsBtn) copyPromptsBtn.style.display = 'block';  
