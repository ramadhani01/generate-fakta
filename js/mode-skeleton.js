...
async function generateSkeleton() {
    ...
    try {
        ...
        // AI mencari ide sendiri dengan format "Berapa lama kamu bisa___?" SAJA
...
- "Berapa lama kamu bisa menahan kencing?"
- "Berapa lama kamu bisa duduk diam tanpa bergerak?"
- "Berapa lama kamu bisa bertahan di ruangan gelap?"
- "Berapa lama kamu bisa menatap layar tanpa berkedip?"
...
Output HANYA ide dalam SATU KALIMAT dengan format "Berapa lama kamu bisa...?"`;
        
        // Minta AI mencari ide
        const ideResponse = await callGroq("Cari ide viral format Berapa lama kamu bisa...", systemCariIde);
        ...
        // Pastikan ide memiliki format "Berapa lama kamu bisa"
        if (!randomIde.toLowerCase().includes('berapa lama kamu bisa')) {
            randomIde = "Berapa lama kamu bisa " + randomIde.toLowerCase();
        }
...
TARGET DURASI: 45-70 detik (setiap scene 4-6 detik)

BUAT NARASI SINGKAT PADAT:

Scene 1 (HOOK):
Tulis ulang judul sebagai pertanyaan, lalu tambahkan 1-2 kalimat hook yang menarik. MAKSIMAL 3 KALIMAT.
...
SETIAP SCENE ESKALASI HANYA 2-3 KALIMAT:
- Efek fisik yang semakin parah (1 kalimat)
- Sensasi tubuh (1 kalimat)
- Perubahan mental (opsional, 1 kalimat)

CONTOH GAYA SINGKAT:
Scene 1:
Berapa lama kamu bisa bertahan di ruangan gelap? Tubuhmu akan memberontak tanpa kamu sadari. Kegelapan total mengacaukan sistem saraf.

Scene 2:
30 menit pertama. Mata buta total mencari cahaya. Pupil membesar maksimal. Kepala mulai pusing berputar.

Gunakan tema: ${randomIde}

STYLE LOCK:
- Bahasa Indonesia SINGKAT dan deskriptif
...
Scene ${jumlahSceneManual}: [narasi]`;
        ...
        const userPrompt = `Buat narasi SINGKAT untuk video 45-70 detik dengan judul: ${randomIde}. ${jumlahSceneManual} scene, masing-masing 2-3 kalimat.`;
        ...
        // Hitung estimasi durasi berdasarkan jumlah kata
        ...
        const totalDurasi = jumlahSceneManual * 5; // 5 detik per scene (estimasi realistis)
        const minutes = Math.floor(totalDurasi / 60);
        const seconds = totalDurasi % 60;
        document.getElementById('sceneInfo').innerHTML = `💀 ${jumlahSceneManual} Scene | ${totalDurasi} dtk | ${randomIde.substring(0, 40)}...`;
...
Output ONE PARAGRAPH IMAGE-TO-VIDEO PROMPT that starts with STYLE LOCK and focuses on MOVEMENT and DURATION:`;
        ...
        updateStats('gen');
        updateStats('succ');
        showNotif(`✅ ${jumlahSceneManual} scene | ${totalDurasi} dtk | ${randomIde.substring(0, 50)}...`);
